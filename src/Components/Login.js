...
function Login() {
  ...
  return (
    <div className="login">
    	<motion.img initial={{opacity: 0, y: '5rem'}} exit={{opacity: 0, y: '5rem'}} animate={{opacity: 1, y: 0}}  layout  src={Icon} className="app__icon"/>
    	<motion.form initial={{opacity: 0, y: '5rem'}} exit={{opacity: 0, y: '5rem'}} animate={{opacity: 1, y: 0}} layout onSubmit={handleSubmit}>
			...
			<p><span>{signup ? 'Already have an account?' : "Don't have an account?"}{' '}</span><b style={signup ? {color: '#5887ff'} : {color: '#f0cf65'}} onClick={()=>{if(!loading){setSignup(!signup)}}}>{!signup ? "Sign Up Now":"Log in Now"}</b></p>
		</motion.form>
    </div>
  )
}
